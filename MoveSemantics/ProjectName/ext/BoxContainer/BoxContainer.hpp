...
public:
	...
	BoxContainer(BoxContainer&& source); // Move Constructor !!!! Project Topic
	~BoxContainer();
	...
	// Helper Methods
	void invalidate(); // Helper Method for Move Operation

	//In class operators
	void operator +=(const BoxContainer<T>& operand);
	void operator =(const BoxContainer<T>& source); // Copy assignment operator
	void operator =(BoxContainer<T>&& source);  // Move Assigment Operator !!!! Project Topic
private : 
...
BoxContainer<T>::BoxContainer(BoxContainer<T>&& source)
{
	...
	//Unbind source from the pointer where the data is stored
	source.invalidate();
}
...
template <typename T>
void BoxContainer<T>::invalidate() {
	m_items = nullptr;
	m_items = 0;
	m_size = 0;
	return;
