...
void STD_Copy::execute() {
  ...
  std::ranges::copy(arr_int03, std::end(vec_int01) - arr_int03.size());
  ...
  std::ranges::copy_if(arr_int03, std::begin(deq_int02),filter);
  ...
  std::cout << std::endl;
  std::cout << "======= std::ranges::copy =======" << std::endl;  
  std::cout << " - A useful function of std::ranges is that you can copy data directly to the output stream with std::ostream_iterator" << std::endl;
  std::cout << "    Note: You will need to flush the buffer to get it out of the ostream (std::flush or std::endl)" << std::endl;
  std::ranges::copy(vec_int01,std::ostream_iterator<int>(std::cout, " "));
  std::cout << std::endl;
}