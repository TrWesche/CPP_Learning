...
class BoxContainer 
{
	...
	static_assert(std::is_default_constructible_v<T>, "Type provided to BoxContainer must have a default constructor for BoxContainer instantiation");
...
template <typename T, typename V>
BoxContainer<T,V>::BoxContainer(const BoxContainer<T,V>& source) requires std::copyable<T>
{
	//Set up the new box
...
// get_max specialization for const char*
template <> 
