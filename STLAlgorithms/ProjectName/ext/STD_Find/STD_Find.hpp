...
void STD_Find::execute() {
  ...
  std::vector<int>::iterator find_result_vec = std::find(std::begin(vec_int01), std::end(vec_int01), target_value);
  ...
  std::deque<int>::iterator find_result_deq = std::find(std::begin(deq_int02), std::end(deq_int02), target_value);
  ...
  std::array<int, 5>::iterator find_if_result_arr = std::find_if(std::begin(arr_int03), std::end(arr_int03), matcher_func);
