...
void STD_ForEach::execute() {
  ...
  std::cout << std::endl;
  std::cout << " ------ For Each can also be used for accumulation, for example summing all the values in a collection (Functor)" << std::endl;
  std::cout << "Summing Values(vec_int01): " << std::endl;
  print_collection_foreach(vec_int01);
  SumForEach sum;
  sum = std::for_each(vec_int01.begin(), vec_int01.end(), sum);
  std::cout << "Total (vec_int01): " << sum.getSum() << std::endl;
  ...
  print_collection_foreach(deq_int02);
  int lambda_sum{};
  std::for_each(deq_int02.begin(), deq_int02.end(), [&lambda_sum](const int val){lambda_sum += val;});
